...
2. 中间 2 到 5 个小节，每节推进一个明确观点。
3. 结尾落到一个清晰结论、判断或行动建议。
...
注意：`cover` 在工作稿阶段默认应保存“封面提示词”，不要用虚假的本地路径占位。只有在真正提交给 `wxpost-cli` 前，`cover` 才必须被替换成真实的本地图片路径。

- 如果 `cover` 本身就是图片路径，不需要重复生成，除非用户明确要求重做封面。
- 如果 `cover` 是一段提示词，就要在创建草稿前先生成封面图，再把 `cover` 替换成最终本地图片路径。
- 生成封面图时必须使用 `npx @rongyan/image-gen-cli@latest`，除非用户明确说"我自己出图"。
...
- 判断 `cover` 是图片路径还是提示词的标准：值以 `./`、`/` 或 `http` 开头且以 `.jpg/.jpeg/.png/.bmp/.gif` 结尾 → 图片路径；否则一律视为提示词，必须生成
- 若是提示词，必须立即执行命令生成图片，**不得只输出提示词了事**：

```bash
npx @rongyan/image-gen-cli@latest "你的封面提示词" --output ./images/cover-raw.jpg --aspect-ratio 16:9
```

- `image-gen-cli` 当前不直接支持 `2.35:1`，因此生成后需要再裁剪为微信公众号图文封面常用的 `2.35:1`
- 裁剪完成后，把最终图片路径写回 `cover`
- 只有用户明确说"我自己出图"或"不用生成"，才可跳过自动生成，改为只输出提示词和裁剪规格

如果当前环境可用 `humanizer-zh`，在事实核实完成后、写入最终 Markdown 前，用它对正文做去 AI 化处理。如果当前环境没有安装它，就按当前运行环境对应的方式安装；如果暂时无法安装，就参考上游文档中的规则手工处理。去 AI 化只用于消除 AI 腔，不用于增加虚构细节。
